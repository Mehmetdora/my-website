        ...
        <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <section class="panel overflow-hidden">
                <div class="border-b border-white/10 p-6">
                    ...
                    <p class="mt-2 text-sm leading-6 text-slate-400">Description, location, date, photo posts, and records are managed here.</p>
                </div>
                        ...
                        <tbody class="divide-y divide-white/10">
                            @foreach($lifePosts as $lifePost)
                                <tr class="text-slate-300">
                                    ...
                                    <td class="px-6 py-5">
                                        <span class="rounded-md border border-white/10 bg-white/5 px-2 py-1 text-xs font-bold {{ $lifePost->post_type === 'audio' ? 'text-[#5DF8D8]' : 'text-rose-200' }}">
                                            {{ $lifePost->post_type === 'audio' ? 'Record' : 'Photo' }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-5">
                                        @if($lifePost->post_type === 'audio')
                                            {{ $lifePost->audio_name ?: '1 audio file' }}
                                        @else
                                            {{ $lifePost->images_count ?? $lifePost->images->count() }} photo
                                        @endif
                                    </td>
                                    <td class="px-6 py-5">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('admin.life.edit', $lifePost) }}" class="btn-outline min-h-10 px-4">Edit</a>
                                            <form method="POST" action="{{ route('admin.life.destroy', $lifePost) }}" onsubmit="return confirm('Are you sure you want to delete this Life post? Its media file(s) will also be deleted. This cannot be undone.');">
                                                @csrf
                                                @method('DELETE')
